package mcdModelChecker.controller.graphs;

import mcdModelChecker.McdModelCheckerApp;
import mcdModelChecker.controller.exceptions.McdException;
import mcdModelChecker.controller.graphs.translationTemplates.*;
import mcdModelChecker.controller.types.kripke.Relation;
import mcdModelChecker.controller.types.translation.*;
...
import static mcdModelChecker.controller.types.vertex.VertexStatus.COMPLETED;

public class Translate extends McdModelCheckerApp {
        ...
        */
        ...
        public VertexList getTranslatedVertexListNoInterleavings(VertexList origVertexList, int numLoops, Boolean isStepSelected, Boolean prevStep) throws McdException {
        ...
        public VertexList getTranslatedVertexListWithInterleavings(VertexList origVertexList, int loopsNum, Boolean isStepSelected, Boolean prevStep) throws McdException {
            return getTranslatedVertexList(origVertexList, true, loopsNum, isStepSelected, prevStep);
        }

        private VertexList getTranslatedVertexList(VertexList origVertexList, Boolean getInterleavings, int loopsNum, Boolean isStepSelected, Boolean prevStep) throws McdException {
            /*
            if (prevStep) {
                targetStep = model.getSelectedStep() - 1;
            } else {
                targetStep = model.getSelectedStep();
            }
            */
            numSteps = 0;
            numNodesExpanded = 0;
            ...
            if (!isStepSelected) {
                ...
                if (!getInterleavings) {
                    // vertexListToTranslate.setNumTotalSteps(model.getNumTranslationSteps());
                } else {
                    // vertexListToTranslate.setNumTotalSteps(model.getNumInterleavingsSteps());
                }
            }
